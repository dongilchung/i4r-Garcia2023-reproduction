...
for(exp_num in 1:length(selected_exp)){
  ...
  models <- paste("modelfitting_LE_fit_exp_",selected_exp[exp_num],sep="")
  ...
  for(chain_index in 1:4 ){
    chain_list[chain_index] <- paste("modelfitting_LE_fit_chain",chain_index,"_exp_",selected_exp[exp_num],".rdata",sep="")
  }
  ...
  png(file=paste("Fig.LE_fit_exp_",selected_exp[exp_num],".trace.png",sep=""), width=600, height=350,unit="px",bg="transparent")
  traceplot(RA_combined,pars=c("mu_p","s_p"));
  dev.off()
  
  Sys.sleep(0.1)
  
  png(file=paste("Fig.LE_fit_exp_",selected_exp[exp_num],".pairs.png",sep=""), width=1000, height=1000,unit="px",bg="transparent")
  pairs(RA_combined,pars=c("mu_p","s_p","lp__"))
  dev.off()
  
  Sys.sleep(0.1)
  
  png(file=paste("Fig.LE_fit_exp_",selected_exp[exp_num],".dense.png",sep=""), width=600, height=350,unit="px",bg="transparent")
  ...
  parameterExtract<-extract(RA_combined,pars=c("alpha1","beta1"))
  medianPosteriorAlpha1 <- apply(parameterExtract$alpha1,2,median)
  medianPosteriorBeta1  <- apply(parameterExtract$beta1,2,median)
  mylog_lik <- extract_log_lik(RA_combined,parameter_name = "log_lik")
  myloo <- loo_power1$looic;
  fname = paste(getwd(),"/posterior_LE_models_5000permute_exp_",selected_exp[exp_num],".mat",sep="");
  writeMat(fname, medianPosteriorAlpha1=medianPosteriorAlpha1, medianPosteriorBeta1=medianPosteriorBeta1,log_lik=mylog_lik, loo_looic=myloo);
}
