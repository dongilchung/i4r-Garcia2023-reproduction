...
selected_exp = c(5, 6.1, 6.2, 7.1 ,7.2, 8.1, 8.2, 9.1, 9.2);
for(exp_num in 1:length(selected_exp)){
  
  models <- paste("modelfitting_EE_fit_exp_",selected_exp[exp_num],sep="")
  ...
  for(chain_index in 1:4 ){
    chain_list[chain_index] <- paste("modelfitting_EE_fit_chain",chain_index,"_exp_",selected_exp[exp_num],".rdata",sep="")
  }
  ...
  png(file=paste("Fig.EE_fit_exp_",selected_exp[exp_num],".trace.png",sep=""), width=600, height=350,unit="px",bg="transparent")
  traceplot(RA_combined,pars=c("mu_p","s_p"));
  dev.off()
  
  Sys.sleep(0.1)
  
  png(file=paste("Fig.EE_fit_exp_",selected_exp[exp_num],".pairs.png",sep=""), width=1000, height=1000,unit="px",bg="transparent")
  pairs(RA_combined,pars=c("mu_p","s_p","lp__"))
  dev.off()

  Sys.sleep(0.1)
    
  png(file=paste("Fig.EE_fit_exp_",selected_exp[exp_num],".dense.png",sep=""), width=600, height=350,unit="px",bg="transparent")
  ...
  fname = paste(getwd(),"/new_posterior_EE_models_5000permute_exp_",selected_exp[exp_num],".mat",sep="");
