...
loadData <- paste(getwd(),"/Data/stanBehav_EE_choices.mat", sep = "");
...
Total_p2Col    <-  allParticipantBehavior$stanAccum.p2.sym;
...
for(exp_num in 1:length(selected_exp)){
  ...
  RA_fit=stan(file = paste(getwd(),"/Estim_EE_Model.stan",sep=""), data=RA_data, verbose=FALSE, warmup=2000, iter=5000, chains=1, init="random", control=list(max_treedepth=15, adapt_delta=0.99, stepsize=0.001))
  fname = paste("modelfitting_EE_fit_chain",ChainNum,"_exp_",selected_exp[exp_num],".rdata",sep="");
